...
class Particle:
    def __init__(self,pdgid,spin,px=0,py=0,pz=0,energy=0,mass=0,mother=0):
        self.pdgid=pdgid
        self.px=px
        self.py=py
        self.pz=pz
    ...
    for child in root:
        if(child.tag=='event'):
            ...
            for i in range(1,num_part+1):
                ...
                p=Particle(int(part_data[0]), float(part_data[12]), float(part_data[6]), float(part_data[7]), float(part_data[8]), float(part_data[9]), float(part_data[10]), [int(part_data[2]), int(part_data[3])])
                e.__addParticle__(p)
